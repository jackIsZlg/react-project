...
// Actions
const QUERY = 'missile/system/QUERY'
const REMOVE = 'missile/system/REMOVE'
const SHOW_MODAL = 'missile/system/SHOW_MODAL'
const HIDE_MODAL = 'missile/system/HIDE_MODAL'
const SAVE = 'missile/system/SAVE'
const ADD = 'missile/system/ADD'

// Action Creators

// 查询
export const query = params => (dispatch, getState) => {
  let newParams = getState().getIn(['system', 'list', 'params']).merge(params).toJS()
  dispatch({
    type: QUERY,
    payload: {
      promise: axios.get('/backend/system/systemQuery', { params: newParams }),
      data: { params }
    }
  })
}
...
export const save = data => dispatch =>
  dispatch({
    type: SAVE,
    payload: axios.post('/backend/system/systemSave', data)
  }).then(() => {
    dispatch(query())
    dispatch(hideModal())
  })
...
dispatch({
  type: ADD,
  payload: axios.post('/backend/system/systemAdd', data)
}).then(() => {
  dispatch(query())
  dispatch(hideModal())
})
...
dispatch({
  type: REMOVE,
  payload: axios.post('backend/system/systemRemove', { id })
}).then(() => {
  dispatch(query())
})
