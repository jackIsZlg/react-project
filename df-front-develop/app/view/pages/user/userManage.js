...
import SelectTag from '../../components/Login/SelectTag'
...
function checkName() {
  ...
  if (nickname.length > 45) {
    new base.warningTip($nameInput[0], '昵称应为1-45字符')
    return false
  }
  return true
}
    ...
    return (
      <div className='bindwechat' onClick={this.handleCancel.bind(this)}>
        <div className="bind-page">
          ...
          <div className="tip-message">注意：继续绑定将丢失微信号信息</div>
        </div>
      </div>
    )
  }
    ...
    switch (targetObjName) {
      ...
      case 'add':
        if (currentObjName1 === 'user-address') {
          ...
          ReactDOM.render(
            <SelectTag mode="edit"
              hasSelect={userInfo.favTag}
              onFinish={this.getTag}
            />,
            ...
            <div ref={el => this.userIntrodution = el} className="user-default user-introdution">
              ...
              <div className="EditStatus-container">
                <div className="kuang">
                  ...
                  />
                  <span className={classNames('word-tip', {'red': this.state.introdution.length > 200 })}>{this.state.introdution ? this.state.introdution.length : 0}
                  </span>
                </div>
                <button className="button-container save btn-animation" onClick={this.editInfo}>保存
                </button>
