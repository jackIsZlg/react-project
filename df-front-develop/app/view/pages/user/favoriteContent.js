...
class FolderApp extends React.Component {
  ...
  getFolderInfo() {
    ...
    base.ajaxList.getFolderInfo(folderInfo.id, (d) => {
      let {id, name, comment, tagArray, collect, shared, isPrivate, postCount, viewCount, owned, otherFolders, creator, createdAt, folderCount, mediaUrls} = d.result
      document.title = name + document.querySelector('title').innerText

      newFolderInfo.id = id
      newFolderInfo.name = name
      newFolderInfo.intro = comment || ''
      newFolderInfo.folderCount = folderCount || 0
  ...
  openMoveCopyPop(type) {
    ...
    let idList = this.idArray(type === 'move' ? 'favoriteId' : 'id')[`${clickAllClick ? 'unIdlist' : 'idlist'}`]
  ...
  // 批量取消精选
  cancelFav() {
    const {isPost, selectNum, selectList, newFolderInfo, clickAllClick} = this.state
    let result = []
    if (isPost) {
      return
    }
    ...
    window.df_confirm({
      ...
      success: () => {
        let {idlist, unIdlist} = self.idArray('favoriteId')
        ...
        for (let i = 0, len = selectList.length; i < len; i += 100) {
          result.push({
            unSelectedIdList: clickAllClick ? unIdlist.slice(i, unIdlist.length).join(',') : '',
            favoriteIdList: (selectNum === newFolderInfo.num || clickAllClick) ? '' : idlist.slice(i, i + 100).join(','),
            folderId: clickAllClick ? folderInfo.id : ''
          })
        }
      ...
      <div className="folder-page">
        ...
        <div id="folder-header-wrapper">
          <FolderHeader folderInfo={newFolderInfo}
            ...
            batchSelected={this.batchSelected.bind(this)}
                  ...
                  newFolderInfo.otherFolders && newFolderInfo.otherFolders.map((item, index) => {
                    ...
                    return (
                      <a key={key}
                        ...
                        href={`/users/favorite-content/${item.id}`}
