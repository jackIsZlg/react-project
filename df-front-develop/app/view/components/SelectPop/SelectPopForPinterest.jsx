      ...
      <div className="left pane">
        <div className="left-holder">
          <div className="left-wrapper">
            <div className="img-area" style={{overflow: 'auto'}}>
...
class RightPane extends React.Component {
  ...
  addFolderAjax(name, id) {
    ...
    base.request({
      url: `${base.baseUrl}/pinterest/post-add`,
      type: 'POST',
      data: {
        // url: encodeURIComponent(self.props.mediaUrl),
        url: self.props.mediaUrl,
      }
    }).done((resultObj) => {
      base.request({
        ...
        type: 'POST',
        data: {
          blogId: resultObj.result,
          folderId: id,
          source: wfTypeData.pinterest.source
        }
      }).done((d) => {
        if (d.success) {
          // ani.success(() => {
...
class SelectPop extends React.Component {
    ...
    $.ajax({
      ...
      d.success && self.setState({
        ...
        let newSysFolderList = self.compareFolderSS(d.result.postTag, self.state.selfFolderList),
          {hasFolder, newSelfFolderList} = self.compareFolderSH(d.result.favoriteList, self.state.selfFolderList)
  ...
  addSystemFolder(sysFolderList, selfFolderList) {
    ...
    let folderNumNow = sysFolderList.length, //
      folderNumNeed = 4 - folderNumNow, // 待补充数量
      originSysFolderLength = originSysFolder.length, // 系统标签剩余长度
      randomArray = base.renderRandomNum(0, originSysFolderLength, folderNumNeed)// 随机数数组
  ...
  render() {
    let data = this.state
    return (<div id="select-pop-panel" className={classNames({'hidden': this.state.hidden})}>
